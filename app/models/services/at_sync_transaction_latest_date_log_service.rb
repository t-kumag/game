class Services::AtSyncTransactionLatestDateLogService
  ...
  def self.get_latest_one(financier_account_type_key, account)
    ...
    when "at_user_card_account_id"
      Entities::AtSyncTransactionLatestDateLog.order(id: :desc).where(at_user_card_account_id: account.at_user_id).pluck("latest_date").first
    when "at_user_bank_account_id"
      Entities::AtSyncTransactionLatestDateLog.order(id: :desc).where(at_user_bank_account_id: account.at_user_id).pluck("latest_date").first
    when "at_user_emoney_service_account_id"
      Entities::AtSyncTransactionLatestDateLog.order(id: :desc).where(at_user_emoney_service_account_id: account.at_user_id).pluck("latest_date").first
    end
  end
