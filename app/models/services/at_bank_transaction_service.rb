class Services::AtBankTransactionService
  ...
  def get_distributed_transactions(account_id)
    ...
    if at_sync_transaction_monthly_log.present?
      next_transaction_used_date = bank.at_user_bank_transactions.order(trade_date: :desc)
                             .where(trade_date: at_sync_transaction_monthly_log.monthly_date..one_day_before_from).first
    end

    transactions[:next_transaction_used_date] = next_transaction_used_date.try(:trade_date) ? next_transaction_used_date.trade_date.strftime('%Y-%m-%d %H:%M:%S') : nil
