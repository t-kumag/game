class Services::AppDeveloperService
  ...
  def app_store_receipt_verification(params)
    begin
      ...
      raise StandardError, 'Error App Store Receipt Verification Response Empty' if res.blank?
...
#
      ...
      when 21002 # TODO 送信したレシートのフォーマットのエラー
        p "receipt NG"
        raise StandardError, 'Error App Store Receipt Verification 21002'
      when 21004 # TODO 購入情報エラー　共有シークレットが間違っている
        p "secret NG"
        raise StandardError, 'Error App Store Receipt Verification 21004'
      when 21007 # TODO prod環境でもstagingの情報を返す場合がある
        p "enviroment NG"
        raise StandardError, 'Error App Store Receipt Verification 21007'
      else
        raise StandardError, 'Error App Store Receipt Verification Status NG'
      end

      unless res['latest_receipt'] != params['receipt_data']
        raise StandardError, 'Error App Store Receipt Verification Receipt NG'
      end

      if res['receipt']['bundle_id'] != "com.osidori.inc"
        raise StandardError, 'Error App Store Receipt Verification Bundle Id NG'
      end
      ...
      res['latest_receipt_info'].each do |r|
        ...
        when "#{Settings.app_store_bundle_id}.familyMonthlySubscription"
          @user.update_rank_premium
          if @user.partner_user.present?
            @user.partner_user.update_rank_premium
          end
  ...
  def google_play_receipt_verification(params)
    begin
      ...
      res = AppDeveloperApiClient.new(requester).request
      raise StandardError, 'Error Google Play Receipt Verification Response Empty' if res.blank?
      ...
      if res['orderId'].blank? || res['startTimeMillis'].blank? || res['expiryTimeMillis'].blank?
        raise StandardError, 'Error Google Play Receipt Verification Empty orderId,startTimeMillis,expiryTimeMillis'
      end
      # 購入情報と購入ログを更新
      save_google_play_purchase_and_purchase_log(res, params['product_id'])

      if @google_play_premium_plans[params['product_id']].blank?
        raise StandardError, 'Error Google Play Receipt Verification Not Found plan'
      end
      if Time.at( res['expiryTimeMillis'].to_i / 1000.0) < Time.zone.now
        raise StandardError, 'Error Google Play Receipt Verification Over Expiry'
      end
  ...
  def save_google_play_purchase_and_purchase_log(row, product_id)
    ...
    params = {
        ...
        start_time_millis: Time.at( row['startTimeMillis'].to_i / 1000.0).strftime('%Y-%m-%d %H:%M:%S'),
        expiry_time_millis: Time.at( row['expiryTimeMillis'].to_i / 1000.0).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    params = {
        ...
        subscription_start_at: Time.at( row['startTimeMillis'].to_i / 1000.0).strftime('%Y-%m-%d %H:%M:%S'),
        subscription_expires_at: Time.at( row['expiryTimeMillis'].to_i / 1000.0).strftime('%Y-%m-%d %H:%M:%S'),
