class Api::V1::Group::GoalsController < ApplicationController
  ...
  def update
    ...
    render json: { errors: { code: '005004', mesasge: "Goal not found." } }, status: 422 and return if before_goal.blank?
    ...
    if goal_setting.blank? || partner_goal_setting.blank?
      render json: { errors: { code: '005005', mesasge: "Goal settings not found." } }, status: 422 and return
    end
  ...
  def destroy
    ...
    goal = Entities::Goal.find_by(id: params[:id], group_id: @current_user.group_id)
    if goal.blank?
      render json: { errors: { code: '005004', mesasge: "Goal not found." } }, status: 422 and return
    end
    begin
      goal.destroy
    rescue ActiveRecord::RecordInvalid => db_err
