class Api::V1::Group::GoalsController < ApplicationController
  ...
  def create
    ...
    begin
      ...
      ActiveRecord::Base.transaction do
        ...
        create_goal_finished_activity(options) if after_create_or_update_exceed_goal_amount?(goal)
      end
  ...
  def update
    ...
    begin
      ActiveRecord::Base.transaction do
        before_update_exceed_goal_amount = before_update_exceed_goal_amount?(goal)
        ...
        create_goal_finished_activity(options) if before_update_exceed_goal_amount && after_create_or_update_exceed_goal_amount?(goal)
      end
    rescue ActiveRecord::RecordInvalid => db_err
      raise db_err
    rescue => exception
  ...
  def add_money
    ...
    if goal_service.check_bank_balance(params[:add_amount], goal_setting)
      ...
      if before_update_exceed_goal_amount && after_create_or_update_exceed_goal_amount?(goal)
  ...
  def before_update_exceed_goal_amount?(goal)
    # 目標貯金総額 > 現在の目標貯金額
    goal.goal_amount >= goal.current_amount
  end

  def after_create_or_update_exceed_goal_amount?(goal)
