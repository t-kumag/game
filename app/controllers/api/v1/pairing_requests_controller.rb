    ...
    begin
      ActiveRecord::Base.transaction do
        ...
        unless @pairing_request
          return render json: { errors: { code: '006006', message: 'pairing token not found.' } }, status: 422
        end

        return render json: { errors: { code: '006007', message: 'paring user not found or invalid token.' } }, status: 422 if DateTime.now > @pairing_request.token_expires_at
        return render json: { errors: { code: '006003', message: 'paring user already exists' } }, status: 422  if @pairing_request.status.to_i == 2
        ...
        return render json: { errors: { code: '006004', message: 'from user group taken' } }, status: 422 if from_user_group.present?
        to_user_group = Entities::ParticipateGroup.find_by(user_id: @current_user.id)
        return render json: { errors: { code: '006005', message: 'to user group taken' } }, status: 422 if to_user_group.present?
