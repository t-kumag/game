class Api::V1::User::PlController < ApplicationController
  ...
  def categories
    share = params[:share] == "true" ? [1] : [0,1]
    @response = Services::PlService.new(@current_user).pl_category_summary(share, nil, nil, params[:page])
    render 'list', formats: 'json', handlers: 'jbuilder'
  end

  def grouped_categories
    share = params[:share] == "true" ? [1] : [0,1]
    @response = Services::PlService.new(@current_user).pl_grouped_category_summary(share, nil, nil, params[:page])
