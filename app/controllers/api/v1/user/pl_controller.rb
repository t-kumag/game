class Api::V1::User::PlController < ApplicationController
  ...
  def summary
    ...
    @response = Services::PlService.new(@current_user).pl_summary(share, params[:page])
    render 'summary', formats: 'json', handlers: 'jbuilder'
  end

  def categories
    share = params[:share] == "true" ? [1] : [0,1]
    @response = Services::PlService.new(@current_user).pl_category_summary(share, params[:page])
