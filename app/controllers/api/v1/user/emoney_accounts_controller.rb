class Api::V1::User::EmoneyAccountsController < ApplicationController
    ...
    def update
      ...
      if @current_user.try(:at_user).try(:at_user_emoney_service_accounts).pluck(:id).include?(account_id)
        ...
        account = Services::FinanceService.new(@current_user).update_account(account, get_account_params(account))
        if account.share
    ...
    def destroy
      ...
      render_disallowed_to_delete_account_ids && return if disallowed_at_emoney_account_ids?([account_id])
