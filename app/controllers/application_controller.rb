class ApplicationController < ActionController::Base
  ...
  def require_group
    render json: { errors: { code: '006001', message: "Require group." } }, status: 422 unless @current_user.group_id.present?
  end
  ...
  def render_disallowed_transaction_ids
    render json: { errors: { code: '004001', message: "Disallowed transaction id." } }, status: 422
  end

  def render_disallowed_goal_ids
    render json: { errors: { code: '005002', message: "Disallowed goal id." } }, status: 422
  end

  def render_disallowed_goal_setting_ids
    render json: { errors: { code: '005003', message: "Disallowed goal setting id." } }, status: 422
  end
