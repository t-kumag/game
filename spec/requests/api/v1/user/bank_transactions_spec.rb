  ...
  describe '#show' do
    context 'success' do
      ...
      it 'response json' do
        ...
        expect(actual_app['amount']).to eq expect_distributed_transaction.amount
        # trade_dateか要確認
        expect(actual_app['used_date']).to eq expect_bank_transaction.trade_date.strftime('%Y-%m-%d %H:%M:%S')
        expect(actual_app['used_location']).to eq expect_distributed_transaction.used_location
        expect(actual_app['user_id']).to eq expect_distributed_transaction.user_id
        expect(actual_app['is_account_shared']).to eq at_user_bank_account.share
        expect(actual_app['is_shared']).to eq expect_distributed_transaction.share
        expect(actual_app['at_transaction_category_id']).to eq expect_distributed_transaction.at_transaction_category_id
        expect(actual_app['category_name1']).to eq expect_distributed_transaction.at_transaction_category.category_name1
        expect(actual_app['category_name2']).to eq expect_distributed_transaction.at_transaction_category.category_name2
        expect(actual_app['transaction_id']).to eq expect_distributed_transaction.at_user_bank_transaction_id
      end
    end
  end
