...
RSpec.describe Api::V2::User::AssetProductsController do
  let(:user) { create(:user, :with_at_user_asset_products) }
  ...
  describe '#index' do
    context 'success' do
      it 'response 200' do
        get "/api/v2/user/stock-accounts/#{at_user_stock_account.id}/asset-products/", headers: headers
